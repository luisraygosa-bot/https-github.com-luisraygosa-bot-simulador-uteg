...
# ==========================================================
# TXT: extraer intención + keywords (GENÉRICO)
# ==========================================================
...
def agrupar_programas(programas: list) -> list:
    ...
    for it in programas:
        ...
        if key not in agrupados:
            agrupados[key] = {**it, "rvoes": [it["rvoe"]]}
        else:
...
def respuesta_definicion_rvoe(audiencia: str):
    ...
    if audiencia == "asesor":
        base += "\n🗣️ “Tu carrera tiene validez oficial porque cuenta con RVOE, eso asegura que tu título es reconocido en México.”"
...
def resolver_seleccion_numero(state: ConversationState, pregunta_norm: str):
...
def buscar_rvoe_especifico(estructura_rvoe, carrera_txt, plantel, incorporacion):
    ...
    carrera_norm = aplicar_sinonimos(normalizar(carrera_txt or ""))
...
# ==========================================================
# Motor principal RVOE (usa state por sesión)
# ==========================================================
def responder_rvoe(state: ConversationState, pregunta: str, doc_rvoe: dict, cache_estructura: dict) -> str:
    pn = aplicar_sinonimos(normalizar(pregunta))
    ...
    if es_seleccion_numero(pn) and state.last_options:
        elegido = resolver_seleccion_numero(state, pn)
        if not elegido:
        ...
        return (
            ...
            responder_formato(state.last_mode, state.last_detail, plantel,
                              elegido["programa"], elegido["incorporacion"],
                              rvoe_principal, rvoes_extra)
        )
    ...
    if DEBUG:
        print("DEBUG:", {"pn": pn, "intent": intent, "plantel": plantel, "incorp": incorporacion, "carrera": carrera})
    ...
    return (
        f"{intro_personalizada(audiencia)}\n\n" +
        responder_formato(state.last_mode, state.last_detail, plantel,
                          it["programa"], it["incorporacion"],
                          rvoe_principal, rvoes_extra)
    )

# ==========================================================
# ENGINE (carga docs una vez)
# ==========================================================
class TaoEngine:
    def __init__(self):
        self.docs = cargar_docs()
    ...
    def answer(self, pregunta: str, session_id: str) -> str:
        doc = buscar_doc_ranking(pregunta, self.docs)
        if not doc:
            return "❌ No encontré un documento que coincida.\n👉 Tip: escribe: rvoe / lista rvoe (plantel) / ayuda"

        state = self.get_state(session_id)

        # Si el doc es de RVOE, usa motor
        if "rvoe" in (doc.get("intencion", "") or "") or "rvoe" in (doc.get("contenido_norm", "") or ""):
            return responder_rvoe(state, pregunta, doc, self.cache_estructura)

        # fallback: devuelve el contenido del doc
...
# API (para Wix)
# ==========================================================
app = FastAPI(title="TAO UTEG API")

# CORS para que Wix pueda llamar tu API desde el navegador
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],   # luego lo podemos restringir a tu dominio Wix
...
@app.get("/health")
def health():
    return {"ok": True, "docs": len(ENGINE.docs)}

@app.post("/ask")
def ask(body: AskBody):
    respuesta = ENGINE.answer(body.pregunta, body.session_id)
