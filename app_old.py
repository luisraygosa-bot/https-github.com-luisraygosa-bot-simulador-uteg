...
# --- Detectar BASE_DIR de forma inteligente ---
# Si existe carpeta "docs" junto a este archivo, úsala. Si no, usa la carpeta raíz del proyecto.
...
# ==========================================================
# WEB: Fuentes externas (UTEG + tu Wix)
# ==========================================================
WEB_DIR = os.path.join(BASE_DIR, "web")
os.makedirs(WEB_DIR, exist_ok=True)

# 👉 Agrega/quita URLs aquí
WEB_URLS = [
    "https://www.uteg.edu.mx/",
    "https://luisraygosa1.wixsite.com/capacitaci/services-3",
]
...
# Si un .txt web fue actualizado hace menos de N horas, lo omitimos (para no descargar siempre)
WEB_MIN_HOURS_BETWEEN_UPDATES = 24
...
STOPWORDS = {
    ...
    "tienen", "tiene", "hay", "dame", "ver",
    "pasame", "pasa", "numero", "porfa", "pls", "porfavor",
    "necesito", "ocupó", "ocupo", "mandame", "mándame",
...
def normalizar(texto: str) -> str:
    ...
    texto = re.sub(r"[^a-z0-9\s/]", " ", texto)  # elimina signos ¿?¡!, etc.
...
# ==========================================================
# WEB: Descarga → texto → .txt en docs/web
# ==========================================================
def _slug_from_url(url: str) -> str:
    ...
    safe = safe.strip("_")[:120] or "pagina"
    return safe

def _extraer_texto_html(html: str) -> str:
...
def _web_txt_path(url: str) -> str:
    name = _slug_from_url(url) + ".txt"
    return os.path.join(WEB_DIR, name)

def _recently_updated(path: str, hours: int) -> bool:
    try:
...
def actualizar_web(force: bool = False):
    print("\n🌐 Actualizando conocimiento desde web...")
    ok = 0
    warn = 0
    skip = 0
    ...
    for url in WEB_URLS:
        out_path = _web_txt_path(url)

        if (not force) and _recently_updated(out_path, WEB_MIN_HOURS_BETWEEN_UPDATES):
            skip += 1
            continue
        ...
        try:
            ...
            # Wix a veces entrega poco por contenido dinámico (JS)
            if len(texto) < 600:
                warn += 1
                print(f"⚠️ Aviso: poco texto detectado (posible contenido dinámico): {url}")

            # Metadata para tu ranking (intención + keywords)
            contenido = (
                "intencion_principal: web_uteg\n"
                "palabras_clave:\n"
                "uteg, universidad, rvoe, validez oficial, inscripcion, becas, titulacion, modalidad, plan, requisitos\n\n"
                f"FUENTE: {url}\n\n"
                f"{texto}\n"
            )
            ...
            ok += 1
            print(f"✅ Guardado: {os.path.relpath(out_path, BASE_DIR)}")
            time.sleep(1.1)

        except Exception as e:
            print(f"❌ Error con {url}: {e}")

    print(f"🌐 Listo: {ok} páginas guardadas | omitidas por reciente: {skip} | avisos: {warn}\n")
    if warn:
        print("📌 Nota: si Wix te sale con poco texto, lo más confiable es copiar/pegar manualmente el contenido a un .txt en docs/web.\n")
...
def ayuda():
    return (
        ...
        "🌐 Web:\n"
        "• actualizar web  (descarga UTEG y tu Wix a docs/web)\n"
        "• actualizar web force  (fuerza la descarga aunque sea reciente)\n\n"
...
def main():
    ...
    while True:
        ...
        # --- Web update ---
        if pn.startswith("actualizar web"):
            force = pn.endswith("force")
            actualizar_web(force=force)
            docs = cargar_docs()
            print(f"📚 Archivos cargados (recargado): {len(docs)}")
            continue

        # --- Buscar doc por ranking ---
        doc = buscar_doc_ranking(pregunta, docs)

        if not doc:
            print("\n❌ No encontré un documento que coincida.")
            print("👉 Tip: escribe ayuda")
            print("👉 Tip: si quieres meter info de UTEG/Wix: 'actualizar web'\n")
            continue

        # Motor RVOE (si detecta doc relacionado)
